#define _XOPEN_SOURCE 500

#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <math.h>
#include <float.h>
#include <sys/types.h>
#include <sys/wait.h>

#include "viajante.h"

//**RODRIGO URREA LOYOLA**
// los comentarios con **...** son los comentarios que yo he agregado
volatile int STATE; //**variable global que indica si se llama a ctrl+C**

void advice(){ //**nueva rutina de atencion**
  STATE = 0; //**se notifica de la llamada a ctrl+C**
}

double viajante(int z[], int n, double **m, int nperm) {
  double min= DBL_MAX; // la distancia mas corta hasta el momento

  STATE = 1; //**se inicia STATE para indicar que aun no ocurre el evento**
  void (*hdlr)() = signal( SIGINT, advice); //**se define la rutina de atencion y se guarda la anterior**
 
  for (int i= 1; i<=nperm && STATE; i++) {  //**se añande STATE a la condicion del for**
    int x[n+1];          // almacenara una ruta aleatoria
    gen_ruta_alea(x, n); // genera ruta x[0]=0, x[1], x[2], ..., x[n], x[0]=0
    // calcula la distancia al recorrer 0, x[1], ..., x[n], 0
    double d= dist(x, n, m);
    if (d<min) {    // si distancia es menor a la mas corta hasta el momento
      min= d;       // d es la nueva distancia mas corta
      for (int j= 0; j<=n; j++)
        z[j]= x[j]; // guarda ruta mas corta en parametro z
  } }

  signal( SIGINT, hdlr); //**se restaura la rutina de atencion previa**
  return min;
}
